...
class TransactionModel {
  ...
  factory TransactionModel.fromJson(Map<String, dynamic> json) {
    if (json.containsKey('item_name')) {
      ...
      final isSale = json['transaction_type'] == 'sale';
      return TransactionModel(
        id: json['id'].toString(),
        titleUrdu: itemName,
  ...
  Map<String, dynamic> toApiJson(int userId) {
    final qtyMatch =
        RegExp(r'^\s*([0-9]+(?:\.[0-9]+)?)\s*(.*)$').firstMatch(tag);
    return {
      'user_id': userId,
      'item_name': titleEnglish,
      'quantity': qtyMatch == null ? null : double.tryParse(qtyMatch.group(1)!),
      'unit': qtyMatch == null || qtyMatch.group(2)!.trim().isEmpty
          ? null
