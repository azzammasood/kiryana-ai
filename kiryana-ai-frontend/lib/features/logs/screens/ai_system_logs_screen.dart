...
class _AiSystemLogsScreenState extends ConsumerState<AiSystemLogsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final session = MockAiLogsData.sessions.firstWhere(
      (s) => s.traceId == widget.traceId,
      orElse: () => MockAiLogsData.sessions.first, // Fallback
    );
                  ...
                  Row(
                    children: [
                      Container(
                        ...
                        child: const Icon(Icons.memory_rounded, color: AppColors.white, size: 24),
                      ),
                      const SizedBox(width: AppSpacing.md),
                      Text(
                        isUrdu ? AppStrings.aiSystemLogsUrdu : AppStrings.aiSystemLogsEnglish,
                        textDirection: isUrdu ? TextDirection.rtl : TextDirection.ltr,
                        style: isUrdu
                          ? TextStyle(fontSize: 24, fontWeight: FontWeight.w700, color: isDark ? AppColors.white : AppColors.primary)
                          : GoogleFonts.inter(fontSize: 24, fontWeight: FontWeight.w700, color: isDark ? AppColors.white : AppColors.primary),
                      ),
                    ],
                  ),
    ...
    bottomNavigationBar: isWide 
          ? null 
          : CustomBottomNav(
              currentIndex: 1, // Logs is technically still the active tab conceptually
              onTap: _onNavTap,
            ),
    );
  }
