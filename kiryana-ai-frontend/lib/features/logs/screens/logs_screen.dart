...
import '../../../../data/mock/mock_logs_data.dart';
import '../../dashboard/widgets/custom_bottom_nav.dart';
import '../../dashboard/widgets/insight_banner.dart';
...
class _LogsScreenState extends ConsumerState<LogsScreen> {
    ...
    return Scaffold(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: ListView(
                        ...
                        children: [
                          const InsightBanner(
                            urduText: AppStrings.sugarInsightUrdu,
                            englishText: AppStrings.sugarInsightEnglish,
                          ),
                          ...
                          if (groups.isEmpty)
                            Container(
                              ...
                              child: Column(
                                children: [
                                  Icon(Icons.receipt_long_rounded, size: 64, color: (isDark ? AppColors.white : AppColors.textHint).withValues(alpha: 0.5)),
                                  ...
                                  Text(
                                    ...
                                    style: GoogleFonts.inter(
                                      ...
                                      color: isDark ? AppColors.white : (isUrdu ? AppColors.textHint : AppColors.textSecondary),
                                    ),
                                  ),
                                ],
                              ),
                            ...
                            ...groups.map((group) => Column(
                              ...
                              children: [
                                ...
                                ...group.items.map((item) => GestureDetector(
                                  onTap: () => context.push('/manual-entry/${item.id}'),
      ...
      floatingActionButton: FloatingActionButton(
        ...
        shape: RoundedRectangleBorder(
          ...
          side: isDark 
              ? BorderSide(color: AppColors.white.withValues(alpha: 0.3), width: 1.2)
              : BorderSide.none,
        ),
  ...
  List<LogGroup> _getGroupedLogs(List<TransactionModel> transactions) {
    ...
    final filtered = transactions.where((t) {
      ...
      if (_selectedTabIndex == 0) {
        // Daily: Today Only
        matchesTab = t.date.year == now.year && t.date.month == now.month && t.date.day == now.day;
      } else if (_selectedTabIndex == 1) {
        // Weekly: Last 7 days
        final weekAgo = startOfToday.subtract(const Duration(days: 7));
        matchesTab = t.date.isAfter(weekAgo) || t.date.isAtSameMomentAs(weekAgo);
      } else {
        // Monthly: Last 30 days
        final monthAgo = startOfToday.subtract(const Duration(days: 30));
        matchesTab = t.date.isAfter(monthAgo) || t.date.isAtSameMomentAs(monthAgo);
      }
      ...
      if (_searchQuery.isNotEmpty) {
        ...
        final matchesTitle = t.titleEnglish.toLowerCase().contains(query) || t.titleUrdu.toLowerCase().contains(query);
    ...
    final groups = groupedMap.entries.map((e) => LogGroup(date: e.key, items: e.value)).toList();
  ...
  String _formatDateKey(DateTime date) {
    ...
    if (date.year == now.year && date.month == now.month && date.day == now.day) {
      return 'Today (${DateFormat('dd MMMM').format(date)})';
    }
    final yesterday = now.subtract(const Duration(days: 1));
    if (date.year == yesterday.year && date.month == yesterday.month && date.day == yesterday.day) {
      return 'Yesterday (${DateFormat('dd MMMM').format(date)})';
    }
